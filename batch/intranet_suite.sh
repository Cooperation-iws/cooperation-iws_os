...
sudo ./cooperation-iws.py -c --createdirectories --isofile /media/partage-sauv/ISO/xubuntu-8.04.1-desktop-i386.iso --modulesfile templates/administrationModulesList_intranet --webappmirror http://localhost:81/depot --debmirror http://localhost:81/deb/mirror/ftp.proxad.net/mirrors/ftp.ubuntu.com/ubuntu/ --debmirrornonfree http://localhost:81/deb/mirror/packages.medibuntu.org/ --cms Joomla --directory /media/partage-dev/1a4 --host administration --password cooperation --username ciwsadmin --artwork ciwsGnome --silent --outputisoname ciws-administration-intranet-0.5.2.iso
