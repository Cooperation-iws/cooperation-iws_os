...
error_reporting(E_ALL ^ E_NOTICE);
	...
	function makeboollist($a) {
		global $entities_renamed, $stem_words;
		while ($char = each($entities_renamed)) {
...
//		$a = remove_accents($a);
	...
	function ignoreword($word) {
		...
		if (strlen($word) < $min_word_length || (!eregi($pattern, remove_accents_renamed($word))) || ($common[$word] == 1)) {
			return 1;
		} else {
			return 0;
		}
